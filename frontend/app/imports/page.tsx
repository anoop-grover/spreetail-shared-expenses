...
export default function ImportsPage() {
  ...
  async function upload() {
    ...
    const result = await api<ImportSession>("/imports/", {
      method: "POST",
      body: formData,
      token,
    });
  ...
  async function review(
    action: "import" | "merge" | "keep_both" | "ignore"
  ) {
    const token = localStorage.getItem("accessToken");
    ...
    const actions = session.anomalies.map((item) => ({
      row_number: item.row_number,
      action,
    }));

    const result = await api<ImportSession>(
      `/imports/${session.id}/review/`,
      {
        method: "POST",
    ...
    <AppShell>
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">
          Parse, validate, detect anomalies, review, import, and report.
        </p>
      </div>
